...
export function registerPeopleTools(server: McpServer, client: SalesMapClient) {
  server.tool(
  ...
  server.tool(
    ...
    async ({ name, organizationId, memo, fieldList }) => {
      try {
  ...
  server.tool(
    ...
    "고객 정보 수정. memo 파라미터에 텍스트를 넣으면 해당 고객에 새 메모가 생성됨 (메모 생성 유일한 방법). fieldList로 커스텀 필드 수정 가능.",
    {
      peopleId: z.string().describe("고객 UUID"),
