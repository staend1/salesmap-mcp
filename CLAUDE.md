...
| `src/index.ts` | MCP 서버 생성 + 46개 tool 등록 |
| `src/client.ts` | SalesMap API 클라이언트 (rate limit 120ms, 429 retry) |
| `src/types.ts` | 공통 타입 + `getClient(extra)` 헬퍼 |
| `src/tools/*.ts` | 19개 tool 파일 (people, org, deal, lead 등) |
...
## 현재 상태
- ✅ 46개 MCP tool 구현 완료
- ✅ Cloudflare → Vercel 마이그레이션 완료
- ✅ 멀티테넌트 (Bearer 토큰 인증) 구현 완료
- ✅ 타입 체크 + Next.js 빌드 통과
- ⬜ Vercel 배포 + 실서버 테스트
- ⬜ MCP Inspector로 전체 tool 통합 테스트
- ⬜ 에러 핸들링 고도화
