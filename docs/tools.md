...
{% hint style="warning" %}
견적서·상품은 이 도구로 생성할 수 없습니다. 견적서는 `salesmap-create-quote`를 사용하세요.
...
| 파라미터                         | 타입     | 필수     | 설명                                                                                         |
| ---------------------------- | ------ | ------ | ------------------------------------------------------------------------------------------ |
| `objectType` | string | ✅ | `people` `organization` `deal` `lead` (또는 한글 `고객` `회사` `딜` `리드`). 커스텀 오브젝트는 **정의 이름을 그대로** 입력 (예: `티켓(CRM)`) |
