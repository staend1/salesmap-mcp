...
> **2026-08-05 — 딜도 개발 중이고 하루이틀 내 나올 예정이라고 확인받았다.**
> 나오면 바로 착수한다. 첫 할 일은 §B-8 검증 시나리오로 딜 지원을 실측하는 것.

#### 🔑 딜이 곧 나온다면 설계가 크게 단순해진다

백엔드는 *"딜만 작은 fallback으로 격리하라"* 고 권고했지만, 그건 **딜 출시가 불투명할 때의
차선책**이다. 하루이틀이면 **v2 경로를 아예 만들지 않는 쪽이 맞다.** 그러면 이 문서의
상당 부분이 통째로 사라진다:

| 항목 | 딜까지 열린 뒤 착수하면 |
|---|---|
| §B-2 `V3_OBJECT_UPDATE` 플래그 + v2 폴백 | **불필요** — 롤백 대상이 없다 |
| §B-4 사용자 필드 양방향 변환 | **한 방향만** — v3의 "이름"으로 통일. UUID→이름 역변환만 남는다 |
| A-4 파이프라인 어휘 역전 (ID↔이름) | **소멸** — v3는 이름 하나뿐 |
| §B-6 v2 순회 주의(thundering herd·부분 실패 정규화) | **소멸** |
| §B-5-5 커오 어휘 충돌 (리터럴↔정의 이름) | **소멸** — v3는 정의 이름 하나뿐 |
| §B-3 properties → data/association 자동 분리 | **그대로 필요** (입력면 하위호환) |

즉 **남는 일은 §B-2 도구 개편 + §B-3 자동 분리 + §B-5 곁다리 수정**이고,
"두 경로를 동시에 유지"하는 복잡도는 전부 없어진다. 보류가 옳았던 이유가 여기 있다.
