...
**쿼타 단위 미확정** — 2026-07-31 회신은 배치도 `per room`이라 했으나, 2026-08-04에
"워크스페이스당이냐 유저당이냐"를 재질의했고 **아직 회신 없음.** 워크스페이스 단위면
같은 워크스페이스의 다른 사용자·다른 MCP 클라이언트와 1 req/s를 나눠 쓰게 되므로,
**우리 쪽 클라이언트 내부 스로틀만으로는 429를 완전히 막을 수 없다** — 429 백오프가
최종 방어선이라는 뜻이다. 회신 오면 이 항목 갱신.
...
>
> 고칠 모양: 엔드포인트별 최소 간격. `/api/v\d+/object/(create|update)` → 1,100ms,
> 그 외 → 현행 120ms. 백엔드 분기 정규식과 **같은 패턴을 쓰는 게 안전하다**
> (`/v3/object/read`·`list`를 같이 묶으면 배치 조회가 100배 느려진다 — 여기가 함정).
