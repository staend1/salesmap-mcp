...
### 4-4. ✅ [대부분 해결] 입력 검증 — 값 파싱 실패 (2026-06)

> **해결 (2026-06)**: 백엔드가 값 검증 단계를 추가 — 잘못된 값이 DB 쿼리까지 내려가기 전에 **400 + 명확한 메시지**로 반환. 기존엔 검증 없이 downstream(쿼리 빌더)에서 파싱이 터져 fallback 500이었음(`getResolvedValue.server.ts`가 값을 그대로 통과).
...
**잔존 가능성**: "대부분" 해결이라, 검증이 안 걸리는 엣지케이스 500이 남을 수 있음. 500 응답은 `reason: null`이라 원인 특정 불가.
**MCP 우회**: search가 그래도 500(`Internal Server Error`)을 받으면 보수적으로 *"값 형식(날짜는 ISO 8601, 숫자는 숫자 값) 확인"* 힌트를 부착(안전망).
...
| 4-4 | Search 값 파싱 실패 → ✅ 400 전환(2026-06), 엣지 500 잔존 가능 | 500 시 "값 형식 확인" 안전망 힌트 | ~3줄 |
