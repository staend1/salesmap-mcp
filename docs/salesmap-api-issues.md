...
### 4-4. 입력 검증 누락 — 값 파싱 실패 시 500 (2026-06 검증)

검색 필터의 **값이 필드 타입으로 파싱되지 않을 때** graceful 검증 없이 500을 던집니다. (연산자·필드 타입 오류는 400으로 정상 처리되지만, 값 파싱 실패만 미검증)

| 입력 | 응답 |
|------|------|
| 날짜 필드에 파싱 불가 문자열 (`생성 날짜` DATE_ON_OR_AFTER `"notadate"`) | `500 Internal Server Error` |
| 숫자 필드에 비숫자 값 (`금액` EQ/GT `"문자열"`) | `500 Internal Server Error` |

→ 값을 파싱하기 전에 타입을 검증해서, 안 맞으면 **500이 아닌 400 + 명확한 메시지**("날짜 형식이 아닙니다" 등)로 돌려줘야 합니다.

**MCP 우회**: 불가. `500 Internal Server Error`는 원인 정보가 없어 `errWithSchemaHint()`도 일반 힌트만 붙일 수 있습니다.
...
| 4-4 | Search 값 파싱 실패 시 500 | errWithSchemaHint() 일반 힌트만 | 우회 불가 (500) |
...
| 15 | 리드→딜 전환 API 없음 | 도구 조합으로 수동 처리 | 이력 이전 불가 |
...
**총 우회 코드: ~510줄** (전체 MCP 서버 코드의 약 30%)
...
6. 리드→딜 전환 API (`POST /v2/lead/{id}/convert`)
