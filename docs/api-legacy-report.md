...
## 13. 커스텀 오브젝트 Definition 목록 조회 API 부재
...
## 14. Engagement 상세 조회 API 대부분 부재
...
| 16 | 노트 생성 API 제한 | 레코드 update의 memo 파라미터 우회 | 날짜/유형/담당자 지정 불가 |
| 17 | 커스텀 오브젝트 Definition 목록 없음 | — | 우회 불가 |
| 18 | Engagement 상세 조회 대부분 404 | email/memo만 인라인 | sms/meeting/alimtalk 불가 |
| 19 | 리드→딜 전환 API 없음 | 도구 조합으로 수동 처리 | 이력 이전 불가 |
