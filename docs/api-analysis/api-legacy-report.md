...
## 22. Engagement 통합 CRUD 부재

### 문제

허브스팟은 engagement(노트, 태스크, 이메일, 미팅 등)를 하나의 통합 API로 CRUD합니다. 세일즈맵은 engagement별로 API가 분산되어 있고, 대부분 미구현입니다.

### HubSpot 비교

```
HubSpot: 3개 Engagement 도구 — 통합 CRUD
  hubspot-create-engagement(type: "NOTE"|"TASK", metadata, associations, ownerId)
    → 노트/태스크를 하나의 도구로 생성. 날짜·담당자·연관 레코드 모두 지정 가능.
  hubspot-get-engagement(engagementId)
    → 모든 engagement 타입을 ID로 조회
  hubspot-update-engagement(engagementId, metadata)
    → 모든 engagement 타입을 수정

참고: EMAIL, CALL, MEETING은 아직 미지원 (create-engagement description에 명시)
```
...
| 기능 | 세일즈맵 API | 비고 |
|------|-------------|------|
| 노트 생성 | 레코드 update의 `memo` 파라미터로 우회 | 날짜/담당자/유형 지정 불가 (#13) |
| 노트 수정 | ❌ 없음 | |
| 태스크 생성 | ❌ `POST /v2/todo` → 500 | |
| 태스크 수정 | ❌ 없음 | |
| 이메일 조회 | ⚠️ 메타데이터만 (본문 없음, #10) | |
| SMS/미팅/알림톡 조회 | ❌ 모두 404 (#15) | |
| Engagement 수정 | ❌ 없음 | |

### MCP에서의 우회

`create-note`(제한적), `read-email`(메타만), `read-memo`, `list-engagements`(타임라인)로 부분 대응. 통합 CRUD는 불가능합니다.
...
| 26 | Engagement 통합 CRUD 없음 | create-note 제한적 우회 | 수정/태스크/통합 불가 |
...
15. 노트 생성 전용 API (날짜·담당자·유형 지정 가능)
16. Engagement 상세 조회 API 확대 (sms, meeting, 알림톡, AI transcript)
17. 리드→딜 전환 API (`POST /v2/lead/{id}/convert`)
18. 필드(Property) 생성·수정 API
19. Association 생성·해제 API
20. Engagement 통합 CRUD (노트/태스크/이메일을 하나의 API로)
