...
세일즈맵 MCP 서버는 19개 도구를 제공하지만, 도구 핸들러 코드의 약 30%(~400줄)가 API 레거시를 우회하는 변환/보정 로직입니다. 이 문서는 각 로직이 **무엇을 하고, 왜 필요하고, API가 개선되면 어떻게 되는지**를 설명합니다.

참고: 각 로직이 대응하는 API 이슈 번호는 `docs/api-analysis/api-mcp-readiness.md`를 참조하세요.
...
**사용 위치**: delete-object(삭제 미리보기), list-engagements

> **2026-04-16 변경**: batch-read는 DEFAULT_PROPERTIES + pickProperties(로직 #2, #18)로 교체. read-note는 14개 필드로 compact 불필요 → 제거. search는 원본 API 응답이 이미 id/name만 반환 → compact 불필요 → 제거.
...
세일즈맵 API는 모든 필드를 항상 반환합니다. 딜 하나에 파이프라인 단계가 5개면, 단계별 진입/체류/퇴장 = 15개 필드가 추가됩니다. 대부분 null.

- `list-engagements`: 활동 타임라인 항목은 type에 따라 관계없는 필드가 많아 null 제거가 의미 있음
- `delete-object` 미리보기: 삭제 전 핵심 필드만 보여주면 충분
...
## 2-b. DEFAULT_PROPERTIES / getDefaultProperties — 기본 반환 필드 정의

**파일**: `src/client.ts` (357~388줄)
**사용 위치**: batch-read-objects (properties 미지정 시)

> **2026-04-16 추가**: HubSpot 방식("properties 미지정 시 기본 서브셋 반환")을 채택하면서 신설.

### 뭘 하는가

`batch-read-objects`에서 `properties`를 명시하지 않으면, 타입별로 미리 정의된 코어 필드 목록을 자동 적용합니다.

```
공통: 이름, 담당자, 팀, 생성 날짜, 수정 날짜
딜: + 금액, 파이프라인, 파이프라인 단계, 상태, 수주 예정일, 마감일
리드: + 금액, 파이프라인, 파이프라인 단계
고객: + 이메일, 전화
회사: (공통만)
커스텀 오브젝트: 동적 감지 (string + required=true + name≠RecordId → 이름 필드로 판단)
```

`properties`를 명시하면 해당 필드만 반환 (기본 필드 포함 안 함 — HubSpot 방식과 동일).

### 왜 필요한가

API가 `properties[]`를 지원하지 않아 전체 필드가 항상 반환됩니다. batch-read 20건 × 50+개 필드 = 1000+개 필드가 LLM 컨텍스트를 낭비합니다. compactRecord(null 제거)로 줄이던 방식에서, 처음부터 코어 필드만 pickProperties로 잘라내는 방식으로 전환.

커스텀 오브젝트는 워크스페이스마다 이름 필드가 다르므로, 필드 스키마를 조회해서 동적으로 이름 필드를 찾습니다. `required: true`인 필드 = API 고정 시스템 필드 (GUI 필수 체크와 무관).

### API가 개선되면

API가 `properties[]` 파라미터를 지원하면, 이 상수와 함수 전체가 삭제됩니다.
...
| DEFAULT_PROPERTIES + getDefaultProperties | ~30줄 | properties[] 파라미터 지원 |
| compactRecord/compactRecords | ~30줄 | properties[] 파라미터 지원 (현재 delete미리보기·list-engagements만 사용) |
